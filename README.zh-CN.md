...
- 默认走 `projection delta-first` 跨机同步，不要求先完整镜像所有 raw session
...
现在的 `sync auto` 会默认优先导出严格的 `projection_delta`。只有在以下三种情况下才会回退到 `projection_full`：

- 本地还没有这个机器的 projection state
- roots 集合发生变化，现有 base snapshot 已经不兼容
- 远端 relay 上找不到做严格 diff 所需的 base snapshot
...
```text
安装并使用这个仓库。为当前机器集合配置 agent-session-vault，使用基于 hostname 的 machine 名称，把 projection delta-first 作为默认同步链路，同时发现 home-level 与 project-level session roots，并在不修改 Tokscale 上游的前提下准备 Tokscale raw 或 canonical 视图。
```

常见 Agent 任务：

- 配置 machine 定义与 root rules
- 执行 `sync auto <machine>` 做 projection delta-first 导入
...
- 默认跨机链路是 `projection delta-first`，raw sync 仍然保留为显式模式
...
- relay 上的 pending bundle 会从当前本地 base snapshot 出发，优先选择最新可直接应用的 bundle，而不是重复回放已经过时的 sibling delta
